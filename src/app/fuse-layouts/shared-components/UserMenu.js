...
const UserMenu = props => {
	...
	return (
		<>
			<Button
				className="h-40"
				style={{ height: '50px', padding: '8px 0 0 10px', margin: '0px', borderRadius: 0 }}
			...
			>
				{user.data.photoURL ? (
					...
					<MuiThemeProvider theme={AvatarStyle}>
						<Avatar style={{ marginTop: '-8px', marginLeft: '8px' }}>{user.data.displayName[0]}</Avatar>
					</MuiThemeProvider>
				)}

				<div
					className="hidden md:flex flex-col mx-0 items-start"
					style={{ marginTop: '-9px', marginLeft: '7px' }}
				>
