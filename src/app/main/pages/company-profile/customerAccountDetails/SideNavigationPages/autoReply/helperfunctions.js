...
function addNodeUnderParent(_ref19) {
  var treeData = _ref19.treeData,
      ...
      insetAtIndex = _ref19.insetAtIndex;
  ...
  var changedTreeData = map({
    ...
    callback: function callback(_ref20) {
      ...
      if(insetAtIndex>=0&&parentNode.children&&parentNode.children.length>insetAtIndex){
        var children = _toConsumableArray(parentNode.children)
        children.splice(insetAtIndex,0,newNode)
      }else{
...
const getParentIndex = (id,grandParent)=>{
    let index = null;

    if(grandParent&&grandParent.children&&grandParent.children.length){
        for(let i=0;i<grandParent.children.length;i++){
            let el = grandParent.children[i];
            if(el.id===id){
                index = i;
                break;
            }
...
module.exports = {addNodeUnderParent,getParentIndex}