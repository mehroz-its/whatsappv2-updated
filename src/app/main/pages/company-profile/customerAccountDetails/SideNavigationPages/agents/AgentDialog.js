...
const AgentDialog = props => {
	...
	const [role, setrole] = React.useState(data.roles?data.roles[0] : null);
	const [currentParams, setCurrentParams] = React.useState({ limit: 100, page: 0 });
	...
	const handleSubmit = () => {
		let params = {
			...
			role: role,
			position: position,
			max_token_count: role === "61" ? -1 : 1 , 
			default_receiver: role === "61" ? true : false
		};
		if (type !== 'update') {
