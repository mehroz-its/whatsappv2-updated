    ...
    return (
        <div>
            <Card style={{ width: '350px', height: 'auto'  }} onClick={() => setIsOpen(true)} className={message.type == 'inbound' ? 'messageRecieve' : 'messageSent'}>
                ...
                <p id="attachmentDate" style={{width: "97%",paddingBottom:'10px',paddingLeft:'10px',fontSize:'10px'}}>{moment(message.dt).format('MMM Do YY, h:mm A')}{message.type === "outbound"  && !message.hideStatus ? MessageStateResolver.resolve(message.status) : null}</p>
