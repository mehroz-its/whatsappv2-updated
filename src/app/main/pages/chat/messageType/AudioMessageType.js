            ...
            <div   style= {message.type == 'inbound' ?{ width: '350px',marginLeft:'5px',paddingBottom:'10px',marginRight:'-6px'}:{ width: '350px',marginLeft:'-12px',paddingBottom:'10px',marginRight:'-6px'}}
                className={message.type == 'inbound' ? 'messageRecieveaudioAlign' : 'messageSent'}>
                    ...
                    message.type === 'inbound' ?
                        <p id="attachmentDate" style={{ width: "100%", paddingLeft:'12px',marginRight: '-19px',color:'black',margin: '-12px',marginTop:'-18px',fontSize: '10px', }}>{moment(message.dt).format('MMM Do YY, h:mm A')}{message.type === "outbound" && !message.hideStatus ? MessageStateResolver.resolve(message.status) : null}</p> :
                        <p id="attachmentDate" style={{ paddingRight:'0px',color:'black',paddingBottom:'2px', margin: '-18px',display: 'flex', justifyContent: 'flex-end', width: "100%", fontSize: '10px' }}>{moment(message.dt).format('MMM Do YY, h:mm A')}{message.type === "outbound"  && !message.hideStatus ? MessageStateResolver.resolve(message.status) : null}</p>
                    } 
                    </div>
