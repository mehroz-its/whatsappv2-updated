            ...
            {display && <Card style={{ width: '350px', height: 'auto' }} className={message.type == 'inbound' ? 'messageRecieve' : 'messageSent'}>
                <div style={{ position: "relative", display: 'inline-block' }}>
                    ...
                    {message.type === 'inbound' ?
                        <p id="attachmentDate" style={{ width: "100%", margin: '10px', fontSize: '10px' }}>{moment(message.dt).format('MMM Do YY, h:mm a')}{message.type === "outbound" && !message.hideStatus  ? MessageStateResolver.resolve(message.status) : null}</p> :
                        <p id="attachmentDate" style={{ paddingRight:'28px' ,display: 'flex', justifyContent: 'flex-end', width: "100%", margin: '10px', fontSize: '10px' }}>{moment(message.dt).format('MMM Do YY, h:mm A')}{message.type === "outbound" && !message.hideStatus  ? MessageStateResolver.resolve(message.status) : null}</p>
                    }
