...
const LocationMessageType = function (props) {
    ...
    return (
        <Card style={{ width: '350px', height: '250px' }} className={message.type == 'inbound' ? 'messageRecieve' : 'messageSent'}>
            ...
            <p style={{ width: "100%", margin: '5px', marginLeft:'0px', fontSize: '10px' }}>{moment(message.dt).format('MMM Do YY, h:mm A')}{message.type === "outbound" && !message.hideStatus  ? MessageStateResolver.resolve(message.status) : null}</p>
